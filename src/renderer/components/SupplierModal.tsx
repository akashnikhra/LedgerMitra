...
export default function SupplierModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.success) {
      setError(res.error || 'Could not save supplier');
      return;
    }
  ...
  async function handleDelete() {
    if (!supplier || !confirm('Delete this supplier? This cannot be undone.')) return;
    const res = await onDelete(supplier.id);
    if (!res.success) {
      setError(res.error || 'Could not delete supplier');
      return;
    }
    onClose();
  }

  const balanceLabel = form.opening_balance_type === 'Cr' ? 'You owe them (Payable)' : 'They owe you (Advance)';
          ...
          <form id="supplier-form" onSubmit={handleSubmit}>
            ...
            <div className="modal-form-row">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  disabled={mode === 'view'}
                >
                  <option value="Cr">Cr — You owe supplier</option>
                  <option value="Dr">Dr — Supplier owes you</option>
                </select>
              </div>
            </div>
            ...
            {supplier && (
              <p className="invoice-outstanding">
                Current balance: <strong>₹{supplier.current_balance.toLocaleString('en-IN')}</strong>
              </p>
            )}
          </form>
        </div>
